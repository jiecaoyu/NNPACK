...
static void FullFill(float input[], float kernel[], float bias[],
    ...
    srand(seed);
    // ramdon full fill input tensor
    for (size_t inputIter = 0;
            inputIter < input_channels * input_size.height * input_size.width;
            inputIter++) {
        input[inputIter] = ((float)(rand()) / RAND_MAX) * 2.0 - 1.0;
    }
...
static void PrintTensor(float tensor[], size_t size[], int dimensions, char name[]) {
    fprintf(stdout, "==================================\n");
    fprintf(stdout, "==> Print Tensor [%s]\n", name);

    if (dimensions == 3) {
        for(size_t iterHeight = 0; iterHeight < size[1] ; iterHeight++) {
            for(size_t iterChannels = 0; iterChannels < size[0] ; iterChannels++) {
                for(size_t iterWidth = 0; iterWidth < size[2] ; iterWidth++) {
                    fprintf(stdout, "%+3.2f ",
                            tensor[ iterChannels * size[1] * size[2] + 
                            iterHeight * size[2] + iterWidth]);
                }
                fprintf(stdout, " | ");
            }
            fprintf(stdout, "\n");
        ...
        for(size_t iterOutChannels = 0; iterOutChannels < size[0] ; iterOutChannels++) {
            for(size_t iterHeight = 0; iterHeight < size[2] ; iterHeight++) {
                for(size_t iterInChannels = 0; iterInChannels < size[1] ;
                        iterInChannels++) {
                    for(size_t iterWidth = 0; iterWidth < size[3] ; iterWidth++) {
                        fprintf(stdout, "%+3.2f ",
                                tensor[ iterOutChannels * size[1] * size[2] * size[3] + 
                                iterInChannels * size[2] * size[3] + 
                                iterHeight * size[3] + iterWidth]);
                    }
                    fprintf(stdout, " | ");
                }
                fprintf(stdout, "\n");
...
struct nnp_profile convolution_check(
        enum mode mode,
        ...
        size_t max_iterations)
{
    ...
    if (mode != mode_inference) {
        fprintf(stderr, "Error: only inference mode supported\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (transform_strategy == nnp_convolution_transform_strategy_precompute) {
        ...
        switch (status) {
            ...
            default:
                fprintf(stderr,
                        "Error: failed to detect transformed kernel size: status %d\n",
                        status);
                exit(EXIT_FAILURE);
        }
    }
    if (transform_strategy == nnp_convolution_transform_strategy_precompute) {
        ...
        if (transformed_kernel == NULL) {
            fprintf(stderr,
                    "Error: failed to allocate %zu bytes for transformed kernel\n",
                    memory_size);
            exit(EXIT_FAILURE);
        }
        ...
        if (status != nnp_status_success) {
            fprintf(stderr,
                    "Error: failed to pre-compute kernel transform: status %d\n",
                    status);
            exit(EXIT_FAILURE);
        }
    ...
    switch (status) {
        ...
        default:
            fprintf(stderr,
                    "Error: failed to detect workspace memory size: status %d\n",
                    status);
            exit(EXIT_FAILURE);
    }
    if (memory_size != 0) {
        ...
        if (memory_block == NULL) {
            fprintf(stderr,
                    "Error: failed to allocate %zu bytes for workspace\n",
                    memory_size);
            exit(EXIT_FAILURE);
        }
    }
    ...
    nnp_convolution_inference(
            ...
            input_channels, output_channels,
            input_size, input_padding, kernel_size, output_subsampling,
            input, transformed_kernel == NULL ? kernel : transformed_kernel, bias,
            output_new,
    ...
    for (size_t outputIter = 0;
            outputIter < output_channels * output_size.height * output_size.width;
            outputIter++) {
        diff += abs(output_new[outputIter] - output_ref[outputIter]);
    }
    if (diff < output_channels * output_size.height * output_size.width * 1e-6) {
...
int main(int argc, char** argv) {
    ...
    void* output_ref = malloc(batch_size * output_channels * output_size.width * output_size.height * sizeof(float));
    void* output_new = malloc(batch_size * output_channels * output_size.width * output_size.height * sizeof(float));
    ...
    memset(output_ref, 0, batch_size * output_channels * output_size.width * output_size.height * sizeof(float));
    memset(output_new, 0, batch_size * output_channels * output_size.width * output_size.height * sizeof(float));
    ...
    convolution_check(
            ...
            input_size, input_padding, kernel_size, output_subsampling,
            input, kernel, bias, output_ref, output_new,
